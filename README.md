# Long-Horizon Conversational Memory — SPARC-LTM

This repository implements a complete **long-horizon conversational memory system** with two phases:

- **Phase 1**: Naive RAG baseline (COMPLETED) — establishes benchmark scores
- **Phase 2**: SPARC-LTM advanced system (COMPLETED) — salience-aware memory management

**SPARC-LTM** = **S**alience and **P**rovenance **A**ware **R**econciliation and **C**ompression for **L**ong-**T**erm **M**emory

The system implements a 4-tier memory architecture (Active, Compressed, Archived, Forgotten) with automatic state transitions, contradiction detection, and provenance tracking.
...
## Phase 2: SPARC-LTM (Advanced System)

Phase 2 implements the complete SPARC-LTM methodology with:

- **4-tier memory architecture**: Active, Compressed (label + archive), Archived, Forgotten
- **Automatic state transitions**: Triggered at 90% capacity
- **Semantic chunking**: Topic-boundary detection via cosine similarity
- **Agentic fact extraction**: LLM extracts atomic claims from chunks
- **Contradiction detection**: Two-pass (FAISS similarity + LLM classifier)
- **Salience scoring**: Multi-factor (entity density, recency, topic importance, uniqueness, frequency)
- **Parallel retrieval**: 4 workers (Dense FAISS, BM25, Compressed labels, Graph traversal)
- **Provenance tracking**: Every fact traces to source dialog IDs
...
- **Complete methodology**: `PHASE2_METHODOLOGY.md` (904 lines)
- **Implementation guide**: `PHASE2_README.md`
- **Architecture diagrams**: See methodology §3, §10
- **Test coverage**: `tests/phase2/` (schemas, store, graph)
